...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, fakeUserID, fakeHostName, env) {
	...
	if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
		...
		if (sub) {
			...
			if (addressesapi.length > 0) {
				订阅器 += `ADDAPI（TLS优选域名&IP 的 API）: <br>&nbsp;&nbsp;${addressesapi.join('<br>&nbsp;&nbsp;')}<br>`;
			} else if (addressesnotlsapi.length > 0) {
				订阅器 += `ADDNOTLSAPI（noTLS优选域名&IP 的 API）: <br>&nbsp;&nbsp;${addressesnotlsapi.join('<br>&nbsp;&nbsp;')}<br>`;
			} else if (addressescsv.length > 0) {
				订阅器 += `ADDCSV（IPTest测速csv文件 限速 ${DLS} ）: <br>&nbsp;&nbsp;${addressescsv.join('<br>&nbsp;&nbsp;')}<br>`;
			} else {
				if (addresses.length > 0) {
					订阅器 += `ADD（TLS优选域名&IP）: <br>&nbsp;&nbsp;${addresses.join('<br>&nbsp;&nbsp;')}<br>`;
				}
				if (addressesnotls.length > 0) {
					订阅器 += `ADDNOTLS（noTLS优选域名&IP）: <br>&nbsp;&nbsp;${addressesnotls.join('<br>&nbsp;&nbsp;')}<br>`;
				}
			}		
		}
		...
		<div class="container">
			...
			<table>
				...
				<tbody>
					<tr>
						<td>自适应</td>
						<td>
							<a href="javascript:void(0)" onclick="copyToClipboard('https://${proxyhost}${hostName}/${uuid}', 'qrcode_0')">https://${proxyhost}${hostName}/${uuid}</a>
							<button class="copy-btn" onclick="copyToClipboard('https://${proxyhost}${hostName}/${uuid}', 'qrcode_0')">点击复制</button>
					...
					<tr>
						<td>Base64</td>
						<td>
							<a href="javascript:void(0)" onclick="copyToClipboard('https://${proxyhost}${hostName}/${uuid}?b64', 'qrcode_1')">https://${proxyhost}${hostName}/${uuid}?b64</a>
							<button class="copy-btn" onclick="copyToClipboard('https://${proxyhost}${hostName}/${uuid}?b64', 'qrcode_1')">点击复制</button>
					...
					<tr>
						<td>Clash</td>
						<td>
							<a href="javascript:void(0)" onclick="copyToClipboard('https://${proxyhost}${hostName}/${uuid}?clash', 'qrcode_2')">https://${proxyhost}${hostName}/${uuid}?clash</a>
							<button class="copy-btn" onclick="copyToClipboard('https://${proxyhost}${hostName}/${uuid}?clash', 'qrcode_2')">点击复制</button>
					...
					<tr>
						<td>Singbox</td>
						<td>
							<a href="javascript:void(0)" onclick="copyToClipboard('https://${proxyhost}${hostName}/${uuid}?sb', 'qrcode_3')">https://${proxyhost}${hostName}/${uuid}?sb</a>
							<button class="copy-btn" onclick="copyToClipboard('https://${proxyhost}${hostName}/${uuid}?sb', 'qrcode_3')">点击复制</button>
			...
			<a href="javascript:void(0);" id="noticeToggle" onclick="toggleNotice()">实用订阅技巧 🔽</a>
			...
			<table>
				...
				<tbody>
					...
					<tr>
						<td>SUBAPI</td>
						<td>${subProtocol}://${subConverter}</td>
					</tr>
					<tr>
						<td>SUBCONFIG</td>
						<td>${subConfig}</td>
			...
			<body>
				...
				<div class="container">
					...
					<div class="notice-content" id="noticeContent">
						<p><strong>1.</strong> ADDAPI 如果是反代IP，可作PROXYIP的请，可添加"proxyip=true"参数添加到末尾，例如：</p>
						<p><code>https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/addressesapi.txt?proxyip=true</code></p>
						<p><strong>2.</strong> ADDAPI 如果是 <a href="https://github.com/XIU2/CloudflareSpeedTest">CloudflareSpeedTest</a> 的 csv 结果文件，例如：</p>
						<p><code>https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/CloudflareSpeedTest.csv</code></p>
						<p><strong>3.</strong> 如需指定2053端口可添加"port=2053"参数添加到末尾，例如：</p>
						<p><code>https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/CloudflareSpeedTest.csv?port=2053</code></p>
						<p><strong>4.</strong> 如需指定筛选条件可添加"id=CF优选"参数添加到末尾，例如：</p>
						<p><code>https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/CloudflareSpeedTest.csv?id=CF优选</code></p>
						<p><strong>5.</strong> 如需指定多参数则需要使用'&'做间隔，例如：</p>
						<p><code>https://raw.githubusercontent.com/cmliu/WorkerVless2sub/main/CloudflareSpeedTest.csv?id=CF优选&port=2053</code></p>
					</div>
					<div class="editor-container">
						<textarea class="editor" id="content" placeholder="请输入优选订阅列表内容..."></textarea>
				...
				<script>
					...
					function saveContent(button) {
						...
						.then(response => {
							...
							saveStatus.textContent = \`已保存 \${now}\`;
					...
					document.addEventListener('DOMContentLoaded', () => {
						...
						textarea.addEventListener('input', () => {
							clearTimeout(timer);
							timer = setTimeout(() => {
								saveContent(document.querySelector('.save-btn'));
							}, 5000);
						});
					});
				</script>
